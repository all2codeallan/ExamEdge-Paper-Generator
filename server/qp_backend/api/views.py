...
class GenerateQuestionPaperView(APIView):
    ...
    def post(self, request):
        try:
            ...
            paper_buffer = BytesIO()
            doc = QuestionPaperGenerator.create_paper(metadata, selected_questions, questions_data)
            doc.save(paper_buffer)
            paper_buffer.seek(0)

            # DEBUG: Save the docx to disk to check if it's valid
            with open(f"/tmp/debug_question_paper_{timestamp}.docx", "wb") as f:
                f.write(paper_buffer.getvalue())

            # Use system zip command for legacy ZipCrypto encryption (Word-compatible)
            with tempfile.TemporaryDirectory() as tmpdir:
                docx_path = os.path.join(tmpdir, f"question_paper_{timestamp}.docx")
                zip_path = os.path.join(tmpdir, f"question_paper_{timestamp}.zip")
                # Write the docx file
                with open(docx_path, "wb") as f:
                    f.write(paper_buffer.getvalue())
                # Use system zip with password (ZipCrypto)
                zip_cmd = [
                    "zip", "-j", "-P", "f2c0", zip_path, docx_path
                ]
                result = subprocess.run(zip_cmd, capture_output=True)
                if result.returncode != 0:
                    return Response({"error": "Failed to create zip: " + result.stderr.decode()}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            ...
            response['Content-Disposition'] = f'attachment; filename="question_paper_{timestamp}.zip"'
            return response
